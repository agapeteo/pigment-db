...
Status: preserved immutable evidence; invalid for protocol-v2 and protocol-v4, previously selected for protocol-v3 buffered acceptance

Protocol-v2 invalidation reason: its universal per-operation schedule did not
match this capture. Protocol v3 restores start-only scheduling only for buffered
baseline/candidate pairs, so this file was its matching buffered comparator.
Protocol v4 requires same-window CPU-pinned recapture, so this file is historical
only. It is not used for physical comparisons and its samples are never merged.
...
The immutable raw sample rows are in `protocol-v1-baseline-20260807.csv`.
